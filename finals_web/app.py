...
def forgot_password_request():
    ...
    try:
        ...
        if not user_email:
            return jsonify({'success': False, 'message': 'No email address found for this account.'}), 404
        ...
        # Return data for EmailJS to send (frontend will handle email sending)
        return jsonify({
            'success': True,
...
@app.route('/seller/balance')
def seller_balance():
    """Seller balance and earnings page"""
    if 'username' not in session or session.get('role') != 'seller':
        return redirect(url_for('login_page'))

    try:
        from firestore_db import orders_ref, users_ref, products_v2_ref, product_variations_ref
        ...
        # Get all delivered orders for this seller (only count delivered orders for balance)
        orders_query = orders_ref.where('seller_username', '==', seller_username).where(
            'status', '==', 'delivered').stream()
        ...
        for order_doc in orders_query:
            ...
            # Get order items from the separate order_items collection
            items_query = db.collection('order_items').where(
                'order_id', '==', order_id).stream()
            ...
            for item_doc in items_query:
                ...
                # Get product details from products_v2
                product_doc = products_v2_ref.document(product_id).get()
                if not product_doc.exists:
                    continue
                ...
                if variation_id:
                    ...
                    if variation_doc.exists:
                        variation_data = variation_doc.to_dict()
                        product_name = variation_data.get(
                ...
                # Calculate seller's actual revenue (deduct 2% admin commission)
                admin_commission = total_price * 0.02
                seller_revenue = total_price - admin_commission
                total_earnings += seller_revenue

                # Use a unique key combining product_id and variation_id
                item_key = f"{product_id}_{variation_id}" if variation_id else product_id

                if item_key not in items_sold_dict:
                    items_sold_dict[item_key] = {
                        'name': product_name,
                        'image': image,
                        'price': unit_price,
                        'quantity_sold': 0,
                        ...
                        'last_sale': order_date
                    }

                items_sold_dict[item_key]['quantity_sold'] += quantity
                items_sold_dict[item_key]['total_revenue'] += seller_revenue

                if order_date < items_sold_dict[item_key]['first_sale']:
                    items_sold_dict[item_key]['first_sale'] = order_date
                if order_date > items_sold_dict[item_key]['last_sale']:
                    items_sold_dict[item_key]['last_sale'] = order_date

        # Convert dict to list and format dates
        items_sold = []
        for item_key, data in items_sold_dict.items():
            data['first_sale'] = data['first_sale'].strftime(
                '%b %d, %Y') if isinstance(data['first_sale'], datetime) else 'N/A'
            data['last_sale'] = data['last_sale'].strftime(
                '%b %d, %Y') if isinstance(data['last_sale'], datetime) else 'N/A'
            items_sold.append(data)

        # Sort by total revenue descending
        items_sold.sort(key=lambda x: x['total_revenue'], reverse=True)

        # Get withdrawal history
        user_doc = users_ref.document(user['id']).get()
        user_data = user_doc.to_dict() if user_doc.exists else {}
        withdrawals = user_data.get('withdrawals', [])

        total_withdrawn = sum(float(w.get('amount', 0))
                              for w in withdrawals if w.get('status') == 'completed')
        total_balance = total_earnings - total_withdrawn
        ...
        print(f"Error loading seller balance: {e}")
        import traceback
        traceback.print_exc()
        flash('Error loading balance information', 'error')
        return redirect(url_for('seller_dashboard'))


@app.route('/api/seller/withdraw', methods=['POST'])
def api_seller_withdraw():
    """Handle seller withdrawal request - DEMO MODE (instantly completes)"""
    if 'username' not in session or session.get('role') != 'seller':
        return jsonify({'success': False, 'message': 'Unauthorized'}), 401

    try:
        from firestore_db import users_ref, orders_ref
        ...
        if amount < 100:
            return jsonify({'success': False, 'message': 'Minimum withdrawal amount is ₱100.00'}), 400
        ...
        # Calculate available balance (using 'delivered' status)
        seller_username = session['username']
        orders_query = orders_ref.where('seller_username', '==', seller_username).where(
            'status', '==', 'delivered').stream()
        ...
        for order_doc in orders_query:
            ...
            for item_doc in items_query:
                item_data = item_doc.to_dict()
                total_price = float(item_data.get('total_price', 0))
                # Deduct 2% admin commission from seller earnings
                admin_commission = total_price * 0.02
                seller_revenue = total_price - admin_commission
                total_earnings += seller_revenue
        ...
        # Create withdrawal request - DEMO MODE: instantly mark as completed
        withdrawal = {
            'amount': amount,
            'method': method,
            ...
            'completed_at': datetime.now(),  # Demo mode: instant completion
            'reference': f"WD-{datetime.now().strftime('%Y%m%d%H%M%S')}"
        }
        ...
        return jsonify({
            'success': True,
            'message': 'Withdrawal completed successfully! (Demo Mode)',
...
@app.route('/seller/balance/report')
def seller_balance_report():
    """Generate CSV report of seller earnings"""
    if 'username' not in session or session.get('role') != 'seller':
        return redirect(url_for('login_page'))
    ...
    try:
        ...
        seller_username = session['username']
        user = get_user_by_username(seller_username)

        # Get all delivered orders (using 'delivered' status)
        orders_query = orders_ref.where('seller_username', '==', seller_username).where(
            'status', '==', 'delivered').stream()
        ...
        for order_doc in orders_query:
            ...
            for item_doc in items_query:
                ...
                # Use unique key for product+variation
                item_key = f"{product_id}_{variation_id}" if variation_id else product_id
        ...
        writer.writerow(['Verdant - Seller Earnings Report'])
        writer.writerow(
            ['Generated:', datetime.now().strftime('%B %d, %Y %I:%M %p')])
        writer.writerow(['Seller:', seller_username])
        writer.writerow([])
        writer.writerow(['SUMMARY'])
        writer.writerow(['Total Earnings:', f"₱{total_earnings:.2f}"])
        writer.writerow(['Total Withdrawn:', f"₱{total_withdrawn:.2f}"])
        writer.writerow(['Available Balance:', f"₱{available_balance:.2f}"])
        writer.writerow([])
        ...
        writer.writerow(['Date', 'Amount', 'Method', 'Status', 'Reference'])

        for w in withdrawals:
            created_at = w.get('created_at')
            date_str = created_at.strftime(
                '%b %d, %Y %I:%M %p') if created_at else 'N/A'
            writer.writerow([
                date_str,
                f"₱{float(w.get('amount', 0)):.2f}",
                w.get('method', 'N/A'),
                w.get('status', 'N/A'),
                w.get('reference', 'N/A')
            ])
        ...
        response.headers[
            'Content-Disposition'] = f'attachment; filename=earnings_report_{datetime.now().strftime("%Y%m%d")}.csv'
        ...
        return redirect(url_for('seller_balance'))
...
def admin_dashboard():
    ...
    try:
        ...
        # Calculate admin earnings from delivered orders
        from datetime import datetime, timedelta, timezone
        from firestore_db import orders_ref

        print("DEBUG: Starting admin earnings calculation...")
        ...
        # Get start of current month (timezone-aware)
        now = datetime.now(timezone.utc)
        month_start = datetime(now.year, now.month, 1, tzinfo=timezone.utc)

        for order_doc in delivered_orders:
            delivered_count += 1
            order_data = order_doc.to_dict()
            order_id = order_doc.id
            ...
            print(
                f"DEBUG: Processing delivered order {order_id}, date: {order_date}")

            # Get order items to calculate commission (2% of item prices)
            items_query = db.collection('order_items').where(
                'order_id', '==', order_id).stream()
            ...
            for item_doc in items_query:
                item_count += 1
                item_data = item_doc.to_dict()
                item_total = float(item_data.get('total_price', 0))
                order_subtotal += item_total
                print(f"DEBUG: Item total: ₱{item_total}")

            print(
                f"DEBUG: Order {order_id} - {item_count} items, subtotal: ₱{order_subtotal}")

            # Calculate fees: ₱10 fixed + 2% commission on items
            order_fixed_fee = 10.0
            order_commission = order_subtotal * 0.02
            order_total_fee = order_fixed_fee + order_commission

            print(
                f"DEBUG: Fixed fee: ₱{order_fixed_fee}, Commission: ₱{order_commission}, Total: ₱{order_total_fee}")

            total_earnings += order_total_fee
            fixed_fees += order_fixed_fee
            commission_fees += order_commission

            # Check if order is from this month
            if order_date:
                try:
                    if order_date >= month_start:
                        this_month_earnings += order_total_fee
                        month_orders += 1
                except TypeError:
                    # If comparison fails, assume it's this month
                    this_month_earnings += order_total_fee
                    month_orders += 1

        print(f"DEBUG: Total delivered orders: {delivered_count}")
        print(f"DEBUG: Total earnings: ₱{total_earnings}")
        print(f"DEBUG: Fixed fees: ₱{fixed_fees}")
        print(f"DEBUG: Commission fees: ₱{commission_fees}")
...
@app.route('/api/mobile/signup/user', methods=['POST'])
def api_mobile_signup_user():
    """Register a new user account from the mobile app."""
    data = request.get_json() or {}
    ...
    try:
        ...
        email_q = db.collection('users').where('email', '==', email).where(
            'role', '==', 'user').limit(1).stream()
        if any(True for _ in email_q):
            return jsonify({'success': False, 'message': f'Email "{email}" is already registered'}), 400

        db.collection('users').document(username).set({
            'username': username, 'email': email, 'phone': phone,
            'password': password, 'role': 'user', 'fullname': '', 'address': '',
            'is_active': True, 'is_verified': False, 'is_approved': False,
            'account_status': 'active', 'country': 'Philippines',
            'created_at': SERVER_TIMESTAMP, 'updated_at': SERVER_TIMESTAMP,
        })
        db.collection('user_applications').add({
            'user_id': username, 'username': username, 'email': email, 'phone': phone,
            'first_name': '', 'last_name': '', 'status': 'pending',
            'submitted_at': SERVER_TIMESTAMP, 'source': 'flutter_app',
        })
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500
...
@app.route('/api/mobile/signup/rider', methods=['POST'])
def api_mobile_signup_rider():
    """Register a new rider application from the mobile app."""
    data = request.get_json() or {}
    username = data.get('username', '').strip()
    email = data.get('email', '').strip()
    phone = data.get('phone', '').strip()
    ...
    if not username or not email or not password or not vehicle_type or not license_number:
        return jsonify({'success': False, 'message': 'All fields are required'}), 400
    ...
    try:
        ...
        email_q = db.collection('users').where('email', '==', email).where(
            'role', '==', 'rider').limit(1).stream()
        if any(True for _ in email_q):
            return jsonify({'success': False, 'message': f'Email "{email}" is already registered'}), 400

        db.collection('users').document(username).set({
            'username': username, 'email': email, 'phone': phone,
            'password': password, 'role': 'rider', 'fullname': '', 'address': '',
            'is_active': True, 'is_approved': False, 'vehicle_type': vehicle_type,
            'license_number': license_number, 'account_status': 'active',
            'country': 'Philippines', 'created_at': SERVER_TIMESTAMP, 'updated_at': SERVER_TIMESTAMP,
        })
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500
...
@app.route('/api/mobile/rider/verify', methods=['POST'])
def api_mobile_rider_verify():
    """Submit rider verification application from mobile app."""
    data = request.get_json() or {}
    username = (data.get('username') or '').strip()
    if not username:
    ...
    try:
        ...
        existing = db.collection('rider_applications') \
            .where('username', '==', username) \
            .where('status', '==', 'pending') \
            .limit(1).stream()
        if any(True for _ in existing):
            return jsonify({'success': False, 'already_pending': True, 'message': 'Application already pending'})
        ...
        user_ref.update({
            'fullname': full_name, 'first_name': first_name, 'last_name': last_name,
            'middle_initial': middle_initial, 'address': address, 'phone': phone,
            'email': email, 'plate_number': plate_number, 'license_number': license_number,
            'license_front': license_front, 'license_back': license_back,
            'is_verified': True, 'updated_at': SERVER_TIMESTAMP,
        })

        db.collection('rider_applications').add({
            'username': username, 'full_name': full_name, 'first_name': first_name,
            'last_name': last_name, 'middle_initial': middle_initial,
            'address': address, 'phone': phone, 'email': email,
            'vehicle_type': vehicle_type, 'plate_number': plate_number,
            'license_number': license_number,
            'license_image': license_front,   # admin panel reads this field
...
# ── Mobile API Routes for Riders ──────────────────────────────────────────

@app.route('/api/mobile/rider/accept_delivery', methods=['POST'])
def mobile_rider_accept_delivery():
    ...
    try:
        ...
        shipping_fee = data.get('shipping_fee', 38.0)
        ...
        if order_data.get('status') != 'accepted':
            return jsonify({'success': False, 'message': 'Order is not available for pickup'}), 400
...
@app.route('/api/mobile/rider/update_status', methods=['POST'])
def mobile_rider_update_status():
    """Rider updates order status (picked_up or delivered)"""
    try:
        data = request.get_json() or {}
        order_id = data.get('order_id')
        ...
        if not order_id or not new_status or not rider_username:
            return jsonify({'success': False, 'message': 'Missing required fields'}), 400
...
def mobile_rider_get_deliveries(rider_username):
    ...
    try:
        ...
        orders_query = db.collection('orders').where(
            'rider_username', '==', rider_username).stream()
        ...
        for order_doc in orders_query:
            ...
            # Skip if not in rider workflow statuses
            if status not in ['picking_up', 'picked_up', 'delivered']:
                continue

            # Get buyer info
            buyer_username = order_data.get('username', '')
            buyer = get_user_by_username(buyer_username)
            buyer_name = buyer.get(
                'fullname', buyer_username) if buyer else buyer_username
            ...
            for item_doc in items_query:
                ...
                product_name = ''
                if variation_id:
                    from firestore_db import product_variations_ref, products_v2_ref
                    var_doc = product_variations_ref.document(
                        variation_id).get()
                    if var_doc.exists:
                        prod_doc = products_v2_ref.document(product_id).get()
                        prod_name = prod_doc.to_dict().get('product_name', '') if prod_doc.exists else ''
                        var_name = var_doc.to_dict().get('variation_name', '')
                        product_name = f"{prod_name} - {var_name}"
                else:
                    from firestore_db import products_v2_ref
                    prod_doc = products_v2_ref.document(product_id).get()
                    if prod_doc.exists:
                        product_name = prod_doc.to_dict().get('product_name', '')
            ...
            if status == 'picking_up':
                picking_up.append(order_info)
            elif status == 'picked_up':
                picked_up.append(order_info)
            elif status == 'delivered':
                delivered.append(order_info)
