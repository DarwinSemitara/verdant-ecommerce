...
def register_checkout_routes(app):
        ...
        try:
            ...
            for cart_id in selected_cart_ids:
                ...
                if cart_doc.exists:
                    ...
                    print(
                        f"DEBUG: Cart data user_id: {cart_data.get('user_id')}")
            ...
            for seller_username, items in orders_by_seller.items():
                ...
                seller_notification_ref = db.collection(
    ...
    def seller_orders():
        ...
        try:
            ...
            print(
                f"DEBUG: Seller: {session['username']}, Approved: {seller_approved}")

            # Check if seller is approved
            if not seller_approved:
                flash(
                    'Your seller application is pending approval. You cannot access orders yet.', 'warning')
                return redirect(url_for('seller_dashboard'))

            # Check if this seller has any products in products_v2 (NEW)
            products_query = products_v2_ref.where(
                'seller_username', '==', session['username']).limit(1).stream()
            ...
            if product_count == 0:
                ...
                for order_doc in orders_query:
                    ...
                    print(
                        f"DEBUG: Processing order {order_id} for seller {session['username']}")
                    ...
                    for item_doc in items_query:
                        ...
                        print(
                            f"DEBUG: Order item - product_id: {product_id}, variation_id: {variation_id}")

                        # Get product from V2 collection
                        product_doc = products_v2_ref.document(
                            product_id).get()
        ...
        try:
            ...
            for item_doc in items_query:
                ...
                if variation_id:
                    ...
                    variation_ref = product_variations_ref.document(
                        variation_id)
            ...
            try:
                ...
                for item_doc in items_query:
                    ...
                    if variation_name:
                        product_details.append(
                            f"{product_name} ({variation_name}) x{quantity}")
                    else:
                ...
                if len(product_details) == 1:
                    ...
                    items_list = '\n'.join(
                        [f"  • {detail}" for detail in product_details[:5]])
...
{session['username']}"""
                ...
                print(
                    f"Successfully sent order acceptance message to {order_data['username']}")
    ...
    def user_orders():
        ...
        print(
            f"DEBUG /user/orders session: username={session.get('username')}, role={session.get('role')}")
        ...
        try:
            ...
            for order_doc in orders_query:
                ...
                seller_store_name = seller.get(
                    'store_name', 'Unknown Store') if seller else 'Unknown Store'
                ...
                for item_doc in items_query:
                    ...
                    print(
                        f"DEBUG: Processing order item - product_id: {product_id}, variation_id: {variation_id}")
                    ...
                    if product_doc.exists:
                        product_data = product_doc.to_dict()
                        print(
                            f"DEBUG: Found in products_v2 - keys: {list(product_data.keys())}")
                        ...
                        if variation_id:
                            ...
                            if variation_doc.exists:
                                variation_data = variation_doc.to_dict()
                                print(
                                    f"DEBUG: Variation found - variation_name: {variation_data.get('variation_name')}")
                                # For variations, show variation name only (product name is the parent)
                                product_name = variation_data.get(
                                    'variation_name', 'Unknown Variation')
                                product_image = variation_data.get(
                                    'image', product_data.get('image', ''))
                            else:
                                print(
                                    f"DEBUG: Variation {variation_id} not found")
                                product_name = product_data.get(
                                    'product_name', 'Unknown Product')
                        ...
                        print(
                            f"DEBUG: Final product_name from v2: {product_name}")
                    else:
                        # Fallback to old products collection
                        print(
                            f"DEBUG: Product {product_id} not found in products_v2, trying old collection")
                        old_product = get_product_by_id(product_id)
                        if old_product:
                            product_name = old_product.get(
                                'product_name', 'Unknown Product')
                            product_image = old_product.get('image', '')
                            print(
                                f"DEBUG: Found in old products - name: {product_name}")
                        else:
                            print(
                                f"DEBUG: Product {product_id} not found in any collection")
                            product_name = 'Unknown Product'
                            product_image = ''
    ...
    def get_notifications():
        ...
        try:
            ...
            notifications_query = db.collection('notifications').where(
                'username', '==', session['username']).limit(50).stream()
    ...
    def mark_notification_read(notification_id):
        ...
        try:
            # Get notification
            notif_ref = db.collection(
                'notifications').document(notification_id)
